...
void ImpResolution_Cu(){
	...
	resolution_phiave = 0;
	//x= imp(random) , y = phi(random)
	
	TH2D *hranA = new TH2D("hranA","hranA",100,0,8.4,100,0,2*pi);
	TH2D *hranB = new TH2D("hranB","hranB",100,0,8.4,100,0,2*pi);

	hranA -> SetTitle("imp(ran),phi(ran)");
	for(Int_t t=0; t<100; t++){
		imp = 8.4*trandom->Rndm();
		phi = 2*pi*trandom->Rndm();
		countA = 0;
		countB = 0;
		Double_t participant_A[64] = {0,};
		...
		for(countA =0; countA<A_Cu; countA++){
			...
			if (participant_A[countA] == 1) { hranA -> Fill(imp,phi);
			}
		}
		for(countB =0; countB<A_Cu; countB++){
			count_pB += participant_B[countB];
			...
			if (participant_B[countB] == 1){ hranB -> Fill(imp, phi);
			}
		}
