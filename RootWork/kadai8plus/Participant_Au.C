	...
	for(Int_t n=1; n<130; n++){
		...
		for(countA =0; countA<A_Cu; countA++){
			...
			if (participant_A[countA] == 1) {
				...
				if(imp == 4.2) hposA2 -> Fill(posA[0][countA], posA[1][countA],posA[2][countA]);
				if(imp == 8.4) hposA3 -> Fill(posA[0][countA], posA[1][countA],posA[2][countA]);
			}
		}
		for(countB =0; countB<A_Cu; countB++){
			...
			if (participant_B[countB] == 1){ 
				...
				if(imp == 4.2) hposB2 -> Fill(posB[0][countB], posB[1][countB],posB[2][countB]);
				if(imp == 8.4) hposB3 -> Fill(posB[0][countB], posB[1][countB],posB[2][countB]);
			}
		}
